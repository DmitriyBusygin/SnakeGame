        ...
        if ((snake.snakeY[0] == apple.getPosY()) && (snake.snakeX[0] == apple.getPosX())) {
            ...
            while (flag) {
                for (int i = 1; i <= snake.lengthSnake; i++) {
                    // если яблоко появилось в клетке занятой змеей, то создать новое яблоко. Иначе все координаты проверены и выходим из цикла
                    if ((snake.snakeX[0] == apple.getPosX() && (snake.snakeY[0] == apple.getPosX()))) {
                        apple.setRandomPosition();
                    } else {
                        flag = false;
                    }
